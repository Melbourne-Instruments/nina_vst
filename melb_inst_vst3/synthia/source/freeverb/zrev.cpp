...
// [s] ~70[m]
const fv3_float_t FV3_(zrev)::delayLengthReal[] = {
    .153129,
    .210389,
    .127837,
    .256891,
...
const fv3_float_t FV3_(zrev)::delayLengthDiff[] = {
    .020346,
    .024421,
    .031604,
    .027333,
...
FV3_(zrev)::FV3_(zrev)() noexcept(false) {
    rt60 = 2.0;
    apfeedback = 0.6;
    loopdamp = 3600;
    outputlpf = 10000;
...
void FV3_(zrev)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) noexcept(false) {
    if (numsamples <= 0)
        return;
...
void FV3_(zrev)::setloopdamp(fv3_float_t value) {
    ...
    for (long i = 0; i < FV3_ZREV_NUM_DELAYS; i++)
        _filt1[i].setLPF_BW(loopdamp, getTotalSampleRate());
}
...
void FV3_(zrev)::setFsFactors() {
    ...
    for (long i = 0; i < FV3_ZREV_NUM_DELAYS; i++)
        _delay[i].setsize(p_(Total[i] - Diff[i], getTotalFactorFs()), f_(delay_EXCURSION, getTotalSampleRate()));
    for (long i = 0; i < FV3_ZREV_NUM_DELAYS; i++)
        _diff1[i].setsize(p_(Diff[i], getTotalFactorFs()), f_(delay_EXCURSION, getTotalSampleRate()));
    setrt60(getrt60());
