...
bool FV3_(irmodel3wm)::setLFThreadPriority(int priority) {
    ...
    if (validThread == true)
        ret = SetThreadPriority(lFragmentThreadHandle, priority);
...
void FV3_(irmodel3wm)::loadImpulse(const fv3_float_t *inputL, long size) noexcept(false) {
    suspend();
    EnterCriticalSection(&mainSection);
...
void FV3_(irmodel3w)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
    suspend();
    EnterCriticalSection(&mainSection);
    try {
...
void FV3_(irmodel3w)::setInitialDelay(long numsamples) noexcept(false) {
    EnterCriticalSection(&mainSection);
...
bool FV3_(irmodel3w)::setLFThreadPriority(int priority) {
    bool ret = false;
    EnterCriticalSection(&mainSection);
    if (ir3wmL->setLFThreadPriority(priority))
        ret = true;
    if (ir3wmR->setLFThreadPriority(priority))
        ret = true;
    LeaveCriticalSection(&mainSection);
    return ret;
}
