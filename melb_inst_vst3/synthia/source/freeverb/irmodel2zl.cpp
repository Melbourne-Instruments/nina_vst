...
void FV3_(irmodel2zlm)::loadImpulse(const fv3_float_t *inputL, long size) noexcept(false) {
    if (size <= 0)
        return;
    unloadImpulse();
    try {
...
void FV3_(irmodel2zlm)::processZL(fv3_float_t *inputL, fv3_float_t *outputL, long numsamples) {
    ...
    if (ZLstart == 0) {
        ...
        for (long i = 1; i < (long)fragments.size(); i++) {
            fragments[i]->MULT(blkdelayDL.at(ifftSlot.L, i - 1), swapSlot.L);
        }
    }
    zlOnlySlot.mute();
...
void FV3_(irmodel2zl)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
