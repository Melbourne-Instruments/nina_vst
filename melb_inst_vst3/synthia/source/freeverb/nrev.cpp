...
const long FV3_(nrev)::allpassCo[] = {
    347,
    113,
    37,
    59,
...
FV3_(nrev)::FV3_(nrev)() noexcept(false) {
    hpf = lpfL = lpfR = 0;
    setrt60(1);
    setfeedback(0.7);
    setdamp(0.5);
...
void FV3_(nrev)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) noexcept(false) {
    if (numsamples <= 0)
        return;
