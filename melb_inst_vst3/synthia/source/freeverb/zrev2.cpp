...
// Fs = 34.125 kHz
const long FV3_(zrev2)::iAllpassLCo[] = {
    617,
    535,
    434,
    347,
...
const long FV3_(zrev2)::iAllpassRCo[] = {
    603,
    547,
    416,
    364,
...
FV3_(zrev2)::FV3_(zrev2)() noexcept(false) {
    rt60 = 2.0;
    rt60_f_low = 1.3;
    rt60_f_high = 0.3;
    rt60_xo_low = 500;
...
void FV3_(zrev2)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) noexcept(false) {
    switch (reverbType) {
...
void FV3_(zrev2)::setrt60(fv3_float_t value) {
    ...
    for (long i = 0; i < FV3_ZREV_NUM_DELAYS; i++) {
        ...
        _lsf0[i].setLSF_RBJ(rt60_xo_low,
            FV3_(utils)::R2dB(std::pow((fv3_float_t)10, (fv3_float_t)-3. * (fv3_float_t)(_delay[i].getsize() + _diff1[i].getsize()) / back / rt60_f_low * (1 - rt60_f_low))),
            1, getTotalSampleRate());
        _hsf0[i].setHSF_RBJ(rt60_xo_high,
            FV3_(utils)::R2dB(std::pow((fv3_float_t)10, (fv3_float_t)-3. * (fv3_float_t)(_delay[i].getsize() + _diff1[i].getsize()) / back / rt60_f_high * (1 - rt60_f_high))),
            1, getTotalSampleRate());
    }
}
