...
void FV3_(delay)::setsize(long size) noexcept(false) {
    if (size <= 0)
        return;
...
void FV3_(delaym)::setsize(long size) noexcept(false) {
    setsize(size, 0);
}

void FV3_(delaym)::setsize(long size, long modsize) noexcept(false) {
#ifdef DEBUG
