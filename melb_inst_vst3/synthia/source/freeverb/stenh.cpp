...
FV3_(stenh)::FV3_(stenh)() noexcept(false) {
    currentfs = FV3_REVBASE_DEFAULT_FS;
    RMS = Attack = Release = 0;
    Ratio = 3;
    SoftKnee = 10;
    DepthBPF = 7;
    DepthBRF = 4;
...
void FV3_(stenh)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) {
    for (long i = 0; i < numsamples; i++) {
        ...
        if (gainS < gainD)
            vcaFactor = gainS * width;
        else
            vcaFactor = gainD * width;
...
void FV3_(stenh)::setThreshold(fv3_float_t dB) {
    thres_dB = dB;
    compS.setThreshold(FV3_(utils)::dB2R(thres_dB));
    compD.setThreshold(FV3_(utils)::dB2R(thres_dB));
}

fv3_float_t FV3_(stenh)::getThreshold() { return thres_dB; }

void FV3_(stenh)::setRMS(fv3_float_t msec) {
    RMS = msec;
    updateNRT();
}

void FV3_(stenh)::setAttack(fv3_float_t msec) {
    Attack = msec;
    update();
}

void FV3_(stenh)::setRelease(fv3_float_t msec) {
    Release = msec;
    update();
}
...
void FV3_(stenh)::setRatio(fv3_float_t value) {
    Ratio = value;
    compS.setRatio(Ratio);
    compD.setRatio(Ratio);
}

void FV3_(stenh)::setSoftKnee(fv3_float_t dB) {
    SoftKnee = dB;
    compS.setSoftKnee(SoftKnee);
    compD.setSoftKnee(SoftKnee);
}
