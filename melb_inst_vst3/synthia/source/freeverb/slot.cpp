...
void FV3_(slot)::alloc(long nsize, long nch) noexcept(false) {
    if (nsize <= 0 || nch <= 0)
        return;
    free();
    try {
    ...
    if (slot_bad_alloc == true) {
        data = NULL;
        for (long t = 0; t < ch; t++)
            FV3_(utils)::aligned_free(data[t]);
        delete[] data;
        std::fprintf(stderr, "slot::alloc(%ld, %ld) bad_alloc\n", nsize, nch);
        throw std::bad_alloc();
    }
...
void FV3_(slot)::mute() {
    if (ch == 0 || size == 0 || data == NULL)
        return;
    for (long t = 0; t < ch; t++)
        FV3_(utils)::mute(data[t], size);
}

void FV3_(slot)::mute(long limit) {
    if (ch == 0 || size == 0 || data == NULL || limit < 0)
        return;
    if (limit > size)
        limit = size;
    for (long t = 0; t < ch; t++)
        FV3_(utils)::mute(data[t], limit);
}

void FV3_(slot)::mute(long offset, long limit) {
    if (ch == 0 || size == 0 || data == NULL || offset < 0 || limit < 0)
        return;
    if (offset > size)
        offset = size;
    if (offset + limit > size)
        limit = size - offset;
    for (long t = 0; t < ch; t++)
        FV3_(utils)::mute(data[t] + offset, limit);
}
