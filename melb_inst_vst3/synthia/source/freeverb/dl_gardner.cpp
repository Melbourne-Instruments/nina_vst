...
#include "dl_gardner.hpp"
#include "fv3_ns_start.h"
#include "fv3_type_float.h"

FV3_(dl_gd_largeroom)::FV3_(dl_gd_largeroom)() noexcept(false) {
    setDCC(4);
    setLPF(2600);
    setDecay(0.2);
}
...
void FV3_(dl_gd_largeroom)::setSampleRate(fv3_float_t fs) noexcept(false) {
    FV3_(delayline)::setSampleRate(fs);
    pbidx[0][1] = p_(8);
    pbidx[0][0] = p_(1);
    pbidx[1][1] = p_(12);
    pbidx[1][0] = p_(1) + p_(8) + p_(1);

    pbidx[2][1] = p_(87);
    pbidx[2][0] = p_(1) + p_(8) + p_(1) + p_(12) + p_(2) + p_(17);
    pbidx[3][1] = p_(62);
    pbidx[3][0] = p_(1) + p_(8) + p_(1) + p_(12) + p_(2) + p_(17) + p_(5);

    pbidx[4][1] = p_(120);
    pbidx[4][0] = p_(1) + p_(8) + p_(1) + p_(12) + p_(2) + p_(17) + p_(87) + p_(31) + p_(3);
    pbidx[5][1] = p_(76);
    pbidx[5][0] = p_(1) + p_(8) + p_(1) + p_(12) + p_(2) + p_(17) + p_(87) + p_(31) + p_(3) + p_(5);
    pbidx[6][1] = p_(30);
    pbidx[6][0] = p_(1) + p_(8) + p_(1) + p_(12) + p_(2) + p_(17) + p_(87) + p_(31) + p_(3) + p_(5) + p_(76) + p_(5);
...
FV3_(gd_largeroom)::FV3_(gd_largeroom)() noexcept(false) {
    setroomsize(0.2);
    setdccutfreq(4);
    setdamp(2600);
...
void FV3_(gd_largeroom)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) noexcept(false) {
    if (numsamples <= 0)
        return;
