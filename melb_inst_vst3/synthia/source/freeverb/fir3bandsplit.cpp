...
#include "fir3bandsplit.hpp"
#include "fv3_ns_start.h"
#include "fv3_type_float.h"

FV3_(fir3bandsplit)::FV3_(fir3bandsplit)() noexcept(false) {
    currentfs = 48000;
...
void FV3_(fir3bandsplit)::splitR(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL1, fv3_float_t *outputR1,
    ...
    long numsamples) {
    if (lpfM == NULL || hpfM == NULL || numsamples <= 0)
        return;
...
void FV3_(fir3bandsplit)::allocFilter(long length) noexcept(false) {
    freeFilter();
    filterLength = length;
    try {
...
void FV3_(fir3bandsplit)::setSampleRate(long fs) noexcept(false) {
    currentfs = fs;
    update();
}
...
void FV3_(fir3bandsplit)::setWindow(long w) noexcept(false) {
    WINDOW = w;
    update();
}
...
void FV3_(fir3bandsplit)::setLowFreqDivider(fv3_float_t fc) noexcept(false) {
    fcLow = fc;
    update();
}
...
void FV3_(fir3bandsplit)::setHighFreqDivider(fv3_float_t fc) noexcept(false) {
    fcHigh = fc;
    update();
}
...
void FV3_(fir3bandsplit)::setTransitionBand(fv3_float_t fc) noexcept(false) {
    trBand = fc;
    update();
}
...
void FV3_(fir3bandsplit)::setParameter(fv3_float_t p) noexcept(false) {
    param = p;
    update();
}
...
void FV3_(fir3bandsplit)::setFragmentSize(long size) noexcept(false) {
    if (size > 0) {
        fragmentSize = size;
        update();
    }
...
void FV3_(fir3bandsplit)::setFactor(long val) noexcept(false) {
    if (val > 0) {
        factor = val;
        update();
    }
...
void FV3_(fir3bandsplit)::update() noexcept(false) {
...
void FV3_(fir3bandsplit)::setIR(unsigned o) noexcept(false) {
    allocIR(o);
    update();
}

void FV3_(fir3bandsplit)::allocIR(unsigned o) noexcept(false) {
    freeIR();
    try {
        switch (o) {
