...
FV3_(delayline)::FV3_(delayline)() noexcept(false) {
...
void FV3_(delayline)::setSampleRate(fv3_float_t fs) noexcept(false) {
    currentfs = fs;
}
...
void FV3_(delayline)::setsize(long size) noexcept(false) {
    if (size <= 0)
        return;
    ...
    if (bufsize > 0 && bufsize <= size) {
        for (long i = 0; i < bufsize; i++)
            new_buffer[size - bufsize + i] = at(i);
    }
    if (bufsize > 0 && bufsize > size) {
