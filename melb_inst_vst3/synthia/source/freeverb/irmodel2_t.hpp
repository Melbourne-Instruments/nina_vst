...
class _FV3_(irmodel2m) :
  ...
  protected:
    ...
    fragFFT;
    _FV3_(blockDelay)
    blkdelayDL, blkdelayDR;
    long fifoSize;
    _FV3_(slot)
    fifoSlot, reverseSlot, ifftSlot, swapSlot, restSlot;

  private:
    _FV3_(irmodel2m)
    (const _FV3_(irmodel2m) & x);
    _FV3_(irmodel2m) &operator=(const _FV3_(irmodel2m) & x);
};

class _FV3_(irmodel2) :
