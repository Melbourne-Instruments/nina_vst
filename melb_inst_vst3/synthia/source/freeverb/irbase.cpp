...
void FV3_(irbase)::setInitialDelay(long numsamples) noexcept(false) {
...
unsigned FV3_(irbase)::setFFTFlags(unsigned flags) {
    if (irmL != NULL)
        irmL->setFFTFlags(flags);
    if (irmR != NULL)
        irmR->setFFTFlags(flags);
    return (fftflags = flags);
}
...
void FV3_(irbase)::setSIMD(uint32_t flag1, uint32_t flag2) {
    if (irmL != NULL)
        irmL->setSIMD(flag1, flag2);
    if (irmR != NULL)
        irmR->setSIMD(flag1, flag2);
    simdFlag1 = flag1;
    simdFlag2 = flag2;
}
...
void FV3_(irbase)::processdrywetout(const fv3_float_t *dL, const fv3_float_t *dR, fv3_float_t *wL, fv3_float_t *wR, fv3_float_t *oL, fv3_float_t *oR, long numsamples) {
    if ((processoptions & FV3_IR_SKIP_FILTER) == 0) {
        for (long i = 0; i < numsamples; i++) {
            wL[i] = filter.processL(wL[i]), wR[i] = filter.processR(wR[i]);
        }
    }
    for (long i = 0; i < numsamples; i++) {
    ...
    if ((processoptions & FV3_IR_MUTE_WET) == 0) {
        for (long i = 0; i < numsamples; i++)
            oL[i] += wL[i] * wet1L + wR[i] * wet2L;
        for (long i = 0; i < numsamples; i++)
            oR[i] += wR[i] * wet1R + wL[i] * wet2R;
    }
    if ((processoptions & FV3_IR_MUTE_DRY) == 0) {
        for (long i = 0; i < numsamples; i++)
            oL[i] += delayDL.process(dL[i]) * dry;
        for (long i = 0; i < numsamples; i++)
            oR[i] += delayDR.process(dR[i]) * dry;
    }
}
