...
class _FV3_(allpass) {
  public:
     ...
     */
    void setsize(long size) noexcept(false);
    long getsize();
    void mute();
    ...
    inline _fv3_float_t _get_z(long index) {
        ...
        if (index > bufsize || index <= 0)
            std::fprintf(stderr, "allpass::_get_z(%ld,%ld)!\n", index, bufsize);
#endif
...
class _FV3_(allpassm) {
  public:
     ...
     */
    void setsize(long size) noexcept(false);
    void setsize(long size, long modsize) noexcept(false);
    long getsize();
    long getdelaysize();
...
class _FV3_(allpass2) {
  public:
     ...
     */
    void setsize(long size1, long size2) noexcept(false);
...
#ifdef DEBUG
        if (index > bufsize1 || index <= 0)
            std::fprintf(stderr, "allpass2::_get_z(%ld,%ld)!\n", index, bufsize1);
#endif
...
class _FV3_(allpass3) {
  public:
    ...
    void setsize(long size1, long size2, long size3) noexcept(false);
    void setsize(long size1, long size1mod, long size2, long size3) noexcept(false);
...
#ifdef DEBUG
        if (index > bufsize1 || index <= 0)
            std::fprintf(stderr, "allpass3::_get_z(%ld,%ld)!\n", index, bufsize1);
#endif
    ...
    inline _fv3_float_t process(_fv3_float_t input) {
        if (bufsize1 == 0 || bufsize2 == 0 || bufsize3 == 0)
            return input;
        return _process(input);
    }
    ...
    inline _fv3_float_t _process(_fv3_float_t input) {
        ...
        writeidx1++;
        if (writeidx1 >= bufsize1)
            writeidx1 = 0;
        readidx1++;
        if (readidx1 >= bufsize1)
            readidx1 = 0;
        bufidx2++;
        if (bufidx2 >= bufsize2)
            bufidx2 = 0;
