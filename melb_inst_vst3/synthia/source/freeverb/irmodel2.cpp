...
void FV3_(irmodel2m)::loadImpulse(const fv3_float_t *inputL, long size) noexcept(false) {
    if (size <= 0)
        return;
    unloadImpulse();
...
void FV3_(irmodel2m)::unloadImpulse() {
    ...
    restSlot.free();
    fragFFT.freeFFT();
    for (std::vector<FV3_(frag) *>::iterator i = fragments.begin(); i != fragments.end(); i++)
        delete *i;
    fragments.clear();
}
...
void FV3_(irmodel2m)::processreplace(fv3_float_t *inputL, long numsamples) {
    ...
    if (numsamples > fragmentSize) // divide into fragmentSize pieces
    {
        ...
        for (long i = 0; i < div; i++) {
            processreplace(inputL + i * fragmentSize, fragmentSize);
        }
    ...
    if (fifoSize + numsamples >= fragmentSize) {
        ...
        for (long i = 0; i < (long)fragments.size(); i++) {
            fragments[i]->MULT(blkdelayDL.at(ifftSlot.L, i), swapSlot.L);
        }
...
void FV3_(irmodel2)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
    if (size <= 0 || fragmentSize < FV3_IR_Min_FragmentSize)
        return;
    unloadImpulse();
