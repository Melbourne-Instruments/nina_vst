...
void FV3_(irmodels)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
    impulseSize = size;
    allocImpulse(size);
...
void FV3_(irmodels)::processreplace(const fv3_float_t *inputL, const fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) {
    ...
    for (long i = 0; i < numsamples; i++) {
        ...
        for (long j = 0; j < impulseSize; j++)
            L += impulse.L[j] * offsetL[j];
        for (long j = 0; j < impulseSize; j++)
            R += impulse.R[j] * offsetR[j];
...
void FV3_(irmodels)::allocImpulse(long size) noexcept(false) {
    freeImpulse();
    impulseSize = size;
    try {
