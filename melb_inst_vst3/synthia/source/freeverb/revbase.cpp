...
FV3_(revbase)::FV3_(revbase)() noexcept(false) {
    setwetr(1);
    setdryr(1);
    setwidth(1);
    primeMode = true;
...
void FV3_(revbase)::setSampleRate(fv3_float_t fs) noexcept(false) {
    if (fs <= 0)
        return;
    currentfs = fs;
    setFsFactors();
...
fv3_float_t FV3_(revbase)::limFs2(fv3_float_t fq) {
    if (fq < 0)
        fq = 0;
    if (fq > getTotalSampleRate() / 2)
        fq = getTotalSampleRate() / 2;
    return fq;
}
