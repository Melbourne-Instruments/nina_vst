...
void FV3_(allpass)::setsize(long size) noexcept(false) {
#ifdef DEBUG
    ...
    if (bufsize > 0 && bufsize <= size) {
        ...
        for (long i = 0; i < bufsize; i++)
            new_buffer[prefix_i + i] = this->process(0);
    }
    if (bufsize > 0 && bufsize > size) {
...
void FV3_(allpassm)::setsize(long size) noexcept(false) {
    setsize(size, 0);
}

void FV3_(allpassm)::setsize(long size, long modsize) noexcept(false) {
#ifdef DEBUG
...
void FV3_(allpass2)::setsize(long size1, long size2) noexcept(false) {
#ifdef DEBUG
...
void FV3_(allpass2)::free() {
    if (buffer1 == NULL || bufsize1 == 0 || buffer2 == NULL || bufsize2 == 0)
        return;
    delete[] buffer1;
    delete[] buffer2;
    buffer1 = buffer2 = NULL;
...
void FV3_(allpass3)::setsize(long size1, long size2, long size3) noexcept(false) {
    setsize(size1, 0, size2, size3);
}

void FV3_(allpass3)::setsize(long size1, long size1mod, long size2, long size3) noexcept(false) {
#ifdef DEBUG
