...
void FV3_(irmodel1m)::loadImpulse(const fv3_float_t *inputL, long size) noexcept(false) {
    if (size <= 0)
        return;
    unloadImpulse();
    ...
    try {
        ...
        FV3_(slot)
        impulse; // normalize impulse with fragment length
        impulse.alloc(2 * fragmentSize, 1);
        for (long i = 0; i < size; i++) {
            impulse.L[i] = inputL[i] / (fv3_float_t)(fragmentSize * 2);
        }
        FFTW_(plan)
        planL; // DFT 2^n impulse
...
void FV3_(irmodel1m)::processreplace(fv3_float_t *inputL, long numsamples) {
    ...
    if (fifopt + numsamples >= impulseSize) {
        processSquareReplace(fifo.L + impulseSize);
    }
...
void FV3_(irmodel1m)::processSquareReplace(fv3_float_t *inputL) {
    ...
    FFTW_(execute)
    (planOrigL); // inputL -DFT(replace)-> fftRevr
    ...
    } else // current == impulseSize
    {
        for (long i = 0; i < impulseSize; i++) {
            delayline.L[impulseSize + i] += fftRevr.L[i];
        }
        for (long i = 0; i < impulseSize - 1; i++) {
            delayline.L[i] += fftRevr.L[impulseSize + i];
        }
...
void FV3_(irmodel1)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
    if (size <= 0)
        return;
    unloadImpulse();
    try {
        ...
        FV3_(irmodel1m) *ir = static_cast<FV3_(irmodel1m) *>(irmL);
        if (ir == NULL) {
            throw std::bad_alloc();
        }
...
void FV3_(irmodel1)::processreplace(const fv3_float_t *inputL, const fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) {
    ...
    long div = numsamples / impulseSize;
    for (long i = 0; i < div; i++)
        processreplaceS(inputL + i * impulseSize, inputR + i * impulseSize, outputL + i * impulseSize, outputR + i * impulseSize, impulseSize);
...
void FV3_(irmodel1)::processreplaceS(const fv3_float_t *inputL, const fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) {
    ...
    if ((processoptions & FV3_IR_MONO2STEREO) != 0) {
        for (long i = 0; i < numsamples; i++)
            inputW.L[i] = inputW.R[i] = (inputL[i] + inputR[i]) / 2.0;
    } else {
