...
class _FV3_(fragfft) {
  public:
    ...
    long getSIMDSize();
    void allocFFT(long size, unsigned fftflags) noexcept(false);
    void freeFFT();
...
class _FV3_(frag) {
  public:
    ...
    void setSIMD(uint32_t flag1, uint32_t flag2);
    uint32_t getSIMD(uint32_t select);
    void loadImpulse(const _fv3_float_t *L, long size, long limit, unsigned fftflags) noexcept(false);
    void loadImpulse(const _fv3_float_t *L, long size, long limit, unsigned fftflags, _fv3_float_t *preAllocatedL) noexcept(false);
    void unloadImpulse();
    long getFragmentSize();
    // add size*2, size*2
  ...
  private:
    ...
    _FV3_(MULT_T)
    MULT_M;
    void allocImpulse(long size) noexcept(false);
