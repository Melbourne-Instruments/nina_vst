...
void FV3_(irmodel3m)::loadImpulse(const fv3_float_t *inputL, long size) noexcept(false) {
    if (size <= 0)
        return;
...
void FV3_(irmodel3m)::allocFrags(std::vector<FV3_(frag) *> *to, const fv3_float_t *inputL, long fragSize, long num, long mod, unsigned fftflags, fv3_float_t *preAllocL) noexcept(false) {
    try {
...
void FV3_(irmodel3m)::freeFrags(std::vector<FV3_(frag) *> *v) {
    for (std::vector<FV3_(frag) *>::iterator i = v->begin(); i != v->end(); i++)
        delete *i;
    v->clear();
}

void FV3_(irmodel3m)::allocSlots(long ssize, long lsize) noexcept(false) {
...
void FV3_(irmodel3m)::processZL(fv3_float_t *inputL, long numsamples) {
    ...
    if (Scursor == 0) {
        ...
        for (long i = 1; i < (long)sFragments.size(); i++) {
            sFragments[i]->MULT(sBlockDelayL.get(i - 1), sSwapSlot.L);
        }
    }
    sOnlySlot.mute();
    ...
    if (lFragments.size() > 0) {
        for (long i = 0; i < numsamples; i++) {
            inputL[i] = (sReverseSlot.L + Scursor)[i] + (restSlot.L + Scursor)[i] + (lReverseSlot.L + Lcursor)[i];
        }
    } else {
        for (long i = 0; i < numsamples; i++) {
            inputL[i] = (sReverseSlot.L + Scursor)[i] + (restSlot.L + Scursor)[i];
        }
    }
    ...
    for (long i = Lstep; i < (((long)lFragments.size()) - 1) * Lcursor / lFragmentSize; i++) {
        if (((long)lFragments.size()) > i + 1) {
            lFragments[i + 1]->MULT(lBlockDelayL.get(i), lSwapSlot.L);
        }
        Lstep++;
    }
...
void FV3_(irmodel3)::loadImpulse(const fv3_float_t *inputL, const fv3_float_t *inputR, long size) noexcept(false) {
    if (size <= 0 || getSFragmentSize() < FV3_IR_Min_FragmentSize || getLFragmentSize() < FV3_IR_Min_FragmentSize)
        return;
    FV3_(irmodel3)::unloadImpulse();
