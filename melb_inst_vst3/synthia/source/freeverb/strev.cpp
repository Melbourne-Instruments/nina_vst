...
const long FV3_(strev)::delayCo[] = {
    4453,
    3720,
    4217,
    3163,
};
const long FV3_(strev)::idxLCo[] = {
    266,
    2974,
    1913,
    1996,
...
FV3_(strev)::FV3_(strev)() noexcept(false) {
    setrt60(1);
    setdccutfreq(10);
...
void FV3_(strev)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR, long numsamples) noexcept(false) {
    if (numsamples <= 0)
        return;
    ...
    while (count-- > 0) {
        ...
        outL = delayC_54._get_z(iLC[0]) + delayC_54._get_z(iLC[1]) - allpassC_55_59._get_z(iLC[2]) + delayC_63._get_z(iLC[3]) - delayC_30._get_z(iLC[4]) - allpassC_31_33._get_z(iLC[5]) - delayC_39._get_z(iLC[6]);
        outR = delayC_30._get_z(iRC[0]) + delayC_30._get_z(iRC[1]) - allpassC_31_33._get_z(iRC[2]) + delayC_39._get_z(iRC[3]) - delayC_54._get_z(iRC[4]) - allpassC_55_59._get_z(iRC[5]) - delayC_63._get_z(iRC[6]);
...
void FV3_(strev)::setrt60(fv3_float_t value) {
    ...
    UNDENORMAL(back);
    if (back > 0)
        decay = std::pow((fv3_float_t)10.0, -3 * (fv3_float_t)tankDelay / back);
    else
        decay = 0;
    if (autoDiff) {
