...
class _FV3_(iir_lr2) {
  public:
    void setLPF(_fv3_float_t fc, _fv3_float_t fs) {
        iir1.setLPF_BW(fc, fs);
        iir2.setLPF_BW(fc, fs);
    }

    void setHPF(_fv3_float_t fc, _fv3_float_t fs) {
        iir1.setHPF_BW(fc, fs);
        iir2.setHPF_BW(fc, fs);
    }
...
class _FV3_(noisegen_pink_frac) {
  ...
  private:
    _FV3_(noisegen_pink_frac)
    (const _FV3_(noisegen_pink_frac) & x);
    ...
    void fractal(_fv3_float_t *v, int N, _fv3_float_t H) {
        ...
        while (l > 1) {
            k = N / l;
            for (c = 0; c < k; c++) {
                v[c * l + l / 2] = (v[c * l] + v[((c + 1) * l) % N]) / 2.0 + 2.0 * r * ((_fv3_float_t)std::rand() - (_fv3_float_t)RAND_MAX / 2.0) / (_fv3_float_t)RAND_MAX;
                if (v[c * l + l / 2] < -1.)
                    v[c * l + l / 2] = -1.;
                if (v[c * l + l / 2] > 1.)
                    v[c * l + l / 2] = 1.;
...
class _FV3_(noisegen_gaussian_white_noise_1) {
  public:
    _FV3_(noisegen_gaussian_white_noise_1)
...
class _FV3_(noisegen_gaussian_white_noise_2) {
  ...
  private:
    _FV3_(noisegen_gaussian_white_noise_2)
    (const _FV3_(noisegen_gaussian_white_noise_2) & x);
...
class _FV3_(noisegen_gaussian_white_noise_3) {
  public:
    _FV3_(noisegen_gaussian_white_noise_3)

    () { setParam(FV3_NOISEGEN_GAUSSIAN_WHITE_3_DEFAULT_PRECISION); }
  ...
  private:
    _FV3_(noisegen_gaussian_white_noise_3)
    (const _FV3_(noisegen_gaussian_white_noise_3) & x);
