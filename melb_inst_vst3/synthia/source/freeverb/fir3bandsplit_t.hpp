...
class _FV3_(fir3bandsplit) {
  public:
    ...
    void setSampleRate(long fs) noexcept(false);
    long getSampleRate();
    void setWindow(long w) noexcept(false);
    long getWindow();
    void setLowFreqDivider(_fv3_float_t fc) noexcept(false);
    _fv3_float_t getLowFreqDivider();
    void setHighFreqDivider(_fv3_float_t fc) noexcept(false);
    _fv3_float_t getHighFreqDivider();
    void setTransitionBand(_fv3_float_t fc) noexcept(false);
    _fv3_float_t getTransitionBand();
    void setParameter(_fv3_float_t p) noexcept(false);
    _fv3_float_t getParameter();
    void setFragmentSize(long size) noexcept(false);
    long getFragmentSize();
    void setFactor(long factor) noexcept(false);
    long getFactor();

    void setIR(unsigned o) noexcept(false);
  ...
  private:
    _FV3_(fir3bandsplit)
    (const _FV3_(fir3bandsplit) & x);
    _FV3_(fir3bandsplit) &operator=(const _FV3_(fir3bandsplit) & x);
    void update() noexcept(false);
    void allocFilter(long length) noexcept(false);
    void freeFilter();
    void allocIR(unsigned o) noexcept(false);
    void freeIR();
    long currentfs;
