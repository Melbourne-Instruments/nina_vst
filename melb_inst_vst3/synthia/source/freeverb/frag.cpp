...
void FV3_(fragfft)::allocFFT(long size, unsigned fftflags) noexcept(false) {
#ifdef DEBUG
...
#pragma omp section
        {
            ...
            FFTW_(execute)
            (planRevL);
            for (long i = 0; i < fragmentSize * 2; i++)
                oL[i] += fftOrig.L[i];
        }
#pragma omp section
        {
...
void FV3_(frag)::loadImpulse(const fv3_float_t *L, const fv3_float_t *R, long size, long limit, unsigned fftflags) noexcept(false) {
...
void FV3_(frag)::loadImpulse(const fv3_float_t *L, const fv3_float_t *R, long size, long limit, unsigned fftflags,
    fv3_float_t *preAllocatedL, fv3_float_t *preAllocatedR) noexcept(false) {
#ifdef DEBUG
...
void FV3_(frag)::allocImpulse(long size) noexcept(false) {
    freeImpulse();
    fragmentSize = size;
...
static void MULT_M_D_SSE4(const fv3_float_t *iL, const fv3_float_t *fL, fv3_float_t *oL, long n) {
    void *soL = NULL;
    __asm__ __volatile__("MULT_M_D_SSE4:          \n\t"
...
static void MULT_M_D_SSE2(const fv3_float_t *iL, const fv3_float_t *fL, fv3_float_t *oL, long n) {
    void *soL = NULL;
    __asm__ __volatile__("MULT_M_D_SSE2:            \n\t"
                         "mov    %0, %4           \n\t"
...
void FV3_(frag)::setSIMD(uint32_t simdFlag) {
    if (simdFlag == 0)
        simdFlag = FV3_(utils)::getSIMDFlag();
    MULT_M = MULT_M_FPU;
